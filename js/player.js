...
window.addEventListener('load', () => {
  ...
  if (hostid) {
    ...
    const codeDisplay = resolvedHostId.slice(-4).toUpperCase();
    const inp = document.getElementById('input-code');
    if (inp) {
      inp.value = codeDisplay;
      inp.readOnly = true;
...
function joinGame() {
  ...
  if (!resolvedHostId) { errEl.textContent = 'Missing room link — scan the QR code again.'; return; }
